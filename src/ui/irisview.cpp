...
void IrisView::mousePressEvent(QMouseEvent *event) {
	...
	if(event->button() == Qt::LeftButton ) {
		if(lastSet >= 0 && lastSet <= 11) {
            ...
            int nearestP = nearestPoint(event->pos(), 5);
			if(nearestP < 0 && !ignoreInput) {
                points[lastSet]->change(event->pos(), 3.0);
				points[lastSet]->show();
				lastChanged = -1;
			}
			else if(nearestP >= 0) {
				...
				// a new one
                points[nearestP]->change(event->pos(), 3.0);
...
void IrisView::mouseMoveEvent(QMouseEvent *event) {
    ...
    if(nearestPoint(event->pos(), 5) >= 0) {
        hoverPoint = nearestPoint(event->pos(), 5);
	...
	if (event->buttons() == Qt::LeftButton && (!ignoreInput || lastChanged >= 0) ) {
		...
		if(lastChanged == -1)
            ...
            points[lastChanged]->change(event->pos(), 3.0);
