...
DilationFilter::DilationFilter(int _dimension) : ImageFilter(_dimension) {}

void DilationFilter::generateFilter() {
...
int DilationFilter::outputPixel() {
    bool match = false;
    for (int i = 0; i < dimension*dimension; i++) {
        if (values[i] == 0) {
            // i.e. if any one of the pixels are black
            match = true;
        }
    }
    return match ? 0 : 255;
